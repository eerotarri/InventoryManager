import { getFridgeItemUseCase } from "../../../application/use_cases/get-fridge-item.use-case";
import { FridgeItem } from "../../../entities/models/fridge-item";
...
export async function getFridgeItemController(id: string) {
  const fridgeItem = await getFridgeItemUseCase(id);

  if (!fridgeItem) {
    return undefined;
  }

  return presenter(fridgeItem as FridgeItem);
}
